...
const TextureFormats = {};
{
    ...
    for (const [name, internalFormat, glformat, type, CpuArray, chn, isInteger] of [
        ...
        ['r32i', GL.R32I, GL.RED_INTEGER, GL.INT, Int32Array, 1, true],
        ['rg32f', GL.RG32F, GL.RG, GL.FLOAT, Float32Array, 2],
        ['rg32i', GL.RG32I, GL.RG_INTEGER, GL.INT, Int32Array, 2, true],
    ...
    ]) TextureFormats[name] = {internalFormat, glformat, type, CpuArray, chn, isInteger};
}

function isIntegerFormat(format) {
    return TextureFormats[format].isInteger;
}
...
// will be replaced with format-specific defines
[DEFINES]
...
#else
    #define varying in
    #ifdef INTEGER_OUTPUT
        layout(location = 0) out ivec4 FOut;
        layout(location = 1) out ivec4 FOut1;
        layout(location = 2) out ivec4 FOut2;
        layout(location = 3) out ivec4 FOut3;
        layout(location = 4) out ivec4 FOut4;
        layout(location = 5) out ivec4 FOut5;
        layout(location = 6) out ivec4 FOut6;
        layout(location = 7) out ivec4 FOut7;
    #else
...
function expandCode(code, mainFunc, outVar, isInteger=false) {
    const stripped = stripComments(code).trim();
    if (stripped != '' && stripped.indexOf(';') == -1) {
        code = `${outVar} = ${isInteger ? 'ivec4' : 'vec4'}(${stripped});`
    }
...
// Store current target info for shader compilation
const shaderState = {
    currentTarget: null
};

const expandVP = memoize(code=>expandCode(code, 'vertex', 'VPos'));
const expandFP = memoize(code=>{
    // Check if we're targeting an integer format
    const target = shaderState.currentTarget;
    const isInteger = target && target.format && TextureFormats[target.format].isInteger;
    // console.log(`isInteger: ${isInteger}, target:`, target);
    return expandCode(code, 'fragment', 'FOut', isInteger);
});
...
// Store current target globally (not ideal but works for now)
function linkShader(gl, uniforms, Inc, VP, FP) {
    // console.log("linkshader")
    const defs = [];
    if (Inc) defs.push(Inc);
    let target = uniforms.target;
    // console.log(uniforms)
    // console.log(target)
    if (target) {
        // TODO: layern isn't in here (yet(?))
        const {format, layern} = target;
        if (layern) defs.push('#define TEXTURE_2D_ARRAY');
        if (format == 'depth') defs.push('#define DEPTH_TEXTURE');
        // console.log("intarget")
        // console.log(TextureFormats[format])
        if (TextureFormats[format].isInteger) defs.push('#define INTEGER_OUTPUT');
    }
    const shaderDefs = defs.join('\n');
    
    // Replace [DEFINES] in template with our defines
    const template = glsl_template.replace('[DEFINES]', shaderDefs);
    
    const defined = definedUniforms([template, Inc, VP, FP].join('\n'));
...
function prepareOwnTarget(self, spec) {
    ...
    // Pass the target format information to the shader
    target._format = spec.format;
...
function drawQuads(self, params, target) {
    ...
    // Pass target format to uniforms for shader compilation
    // console.log("drawquads")
    // console.log(target)
    let fmt;
    if (target && (fmt = target._format || target.format || (target[0] && target[0].format))) {
        uniforms.target = { format: fmt };
        // Set current target for expandFP
        shaderState.currentTarget = { format: fmt };
    }
